...
def init_db():
    ...
    with db_conn() as conn:
        ...
        if USING_POSTGRES:
            ...
            """)
            c.execute("""
                CREATE TABLE IF NOT EXISTS saved_group_links (
...
def save_group_link(group_id, group_name, group_link):
    if not group_id or not group_link:
        return
    try:
        with db_conn() as conn:
            c = conn.cursor()
            ...
            if USING_POSTGRES:
                c.execute("""
                    INSERT INTO saved_group_links (group_id, group_name, group_link)
                    VALUES (%s, %s, %s)
                    ON CONFLICT (group_id) DO UPDATE SET group_name = EXCLUDED.group_name, group_link = EXCLUDED.group_link
                """, (group_id, group_name, group_link))
            else:
                c.execute("""
                    INSERT OR REPLACE INTO saved_group_links (group_id, group_name, group_link)
                    VALUES (?, ?, ?)
                """, (group_id, group_name, group_link))
        logger.info(f"💾 Saved group link: {group_name} ({group_id}) -> {group_link}")
    except Exception as e:
        logger.error(f"Error saving group link in DB: {e}")

def extract_and_save_group_link(chat_entity, input_link_str):
    if not chat_entity:
        return
    try:
        from telethon.utils import get_peer_id
        group_id = get_peer_id(chat_entity)
        group_name = getattr(chat_entity, 'title', None) or getattr(chat_entity, 'first_name', None) or str(group_id)
        
        parsed = parse_telegram_link(input_link_str)
        if parsed:
            if parsed["type"] == "invite":
                group_link = f"https://t.me/+{parsed['hash']}"
            else:
                group_link = f"https://t.me/{parsed['username']}"
        else:
            clean = input_link_str.strip().lstrip("@")
            if not clean.replace("-", "").isdigit():
                group_link = f"https://t.me/{clean}"
            else:
                group_link = None
                
        if group_link:
            save_group_link(group_id, group_name, group_link)
    except Exception as e:
        logger.error(f"Error extracting group link: {e}")

async def ensure_joined(client, group_id):
    ...
    try:
        ...
        logger.info(f"Client check failed for group {group_id}: {e}")
        in_chat = False
    ...
    if not in_chat:
        ...
        try:
            with db_conn() as conn:
                c = conn.cursor()
                p = get_placeholder()
                c.execute(f"SELECT group_name, group_link FROM saved_group_links WHERE group_id = {p}", (group_id,))
                row = c.fetchone()
                if row:
                    group_name, group_link = row
        except Exception as e:
            logger.error(f"Error reading saved group link: {e}")
            
        if group_link:
            client_name = getattr(client._me, 'first_name', 'Fallback Client')
            logger.info(f"🔄 Auto-join: Client {client_name} attempting to join {group_id} ({group_name}) via link: {group_link}")
            parsed = parse_telegram_link(group_link)
            if parsed:
                if parsed["type"] == "invite":
                    from telethon.tl.functions.messages import ImportChatInviteRequest
                    try:
                        await client(ImportChatInviteRequest(parsed["hash"]))
                        logger.info(f"✅ Auto-join: Successfully joined invite link for group {group_id}")
                        return True
                    except errors.UserAlreadyParticipantError:
                        return True
                    except Exception as je:
                        logger.error(f"❌ Auto-join: Failed to join invite link {group_link}: {je}")
                else:
                    from telethon.tl.functions.channels import JoinChannelRequest
                    try:
                        chat_entity = await client.get_entity(parsed["username"])
                        await client(JoinChannelRequest(chat_entity))
                        logger.info(f"✅ Auto-join: Successfully joined username/public link for group {group_id}")
                        return True
                    except Exception as je:
                        logger.error(f"❌ Auto-join: Failed to join public link {group_link}: {je}")
            else:
                clean_link = group_link.strip().lstrip("@")
                if not clean_link.replace("-", "").isdigit():
                    from telethon.tl.functions.channels import JoinChannelRequest
                    try:
                        chat_entity = await client.get_entity(clean_link)
                        await client(JoinChannelRequest(chat_entity))
                        logger.info(f"✅ Auto-join: Successfully joined raw username for group {group_id}")
                        return True
                    except Exception as je:
                        logger.error(f"❌ Auto-join: Failed to join raw username {clean_link}: {je}")
        else:
            logger.warning(f"⚠️ Auto-join: No saved link found for group {group_id} in database. Cannot auto-join.")
    return in_chat
    ...
    with db_conn() as conn:
        ...
        for table in tables:
            try:
                ...
                logger.error(f"Backup failed for table {table}: {e}")
    return json.dumps(backup_data, indent=2, default=str)

def restore_db_backup(backup_json_str):
    ...
    with db_conn() as conn:
        ...
        for table, rows in data.items():
            ...
            try:
                ...
                logger.error(f"Failed to clear table {table} before restore: {de}")
                
            for row in rows:
                placeholders = []
                vals = []
                for col in cols:
                    placeholders.append(get_placeholder())
                    vals.append(row[col])
                
                cols_str = ", ".join(cols)
                placeholders_str = ", ".join(placeholders)
                if USING_POSTGRES:
                    placeholders_str = placeholders_str.replace("?", "%s")
                query = f"INSERT INTO {table} ({cols_str}) VALUES ({placeholders_str})"
                try:
                    c.execute(query, tuple(vals))
                except Exception as ie:
                    logger.error(f"Failed to insert row in table {table} during restore: {ie}")
        conn.commit()
        ...
        try:
            ...
            if db_active_id == INSTANCE_ID:
                ...
                if not last_backup:
                    ...
                    try:
                        if now - int(last_backup) >= 86400 * 10:
                            should_backup = True
                    except ValueError:
                        should_backup = True
                ...
                if should_backup:
                    ...
                    try:
                        backup_str = generate_db_backup()
                        filename = f"userbot_backup_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.json"
                        ...
                        with open(filename, "rb") as f:
                            bot.send_document(
                                ADMIN_ID,
                                f,
                                caption="📦 **Automatic 10-Day Database Backup**\n\nTo restore this backup on a new bot instance, reply to this JSON file with `/restore`."
                            )
                        os.remove(filename)
                        logger.info("Daily backup sent to admin successfully.")
                    except Exception as be:
                        logger.error(f"Failed to generate/send automatic backup: {be}")
            else:
                is_recent = False
                if db_heartbeat_str:
                    try:
...
def pair_view_markup(pair_id, show_mirror=False):
    ...
    return markup
async def get_topic_selection_markup(chat_id, prefix, client=None):
    markup = InlineKeyboardMarkup(row_width=1)
    target_client = client or userbot
    ...
    try:
        ...
        entity = await resolve_target_id(target_client, chat_id)
        result = await target_client(functions.messages.GetForumTopicsRequest(
            peer=entity,
            offset_date=0,
            offset_id=0,
            offset_topic=0,
    ...
    back_cb = "pair_add_start"
    if prefix == "sel_src_topic":
        back_cb = "pair_userbots_back|sel_src"
    elif prefix == "sel_tgt_topic":
        back_cb = "pair_userbots_back|sel_tgt"
    markup.add(InlineKeyboardButton("🔙 Back to Chats", callback_data=back_cb))
    return markup
...
async def get_chat_selection_markup(prefix, page=0, client=None):
    markup = InlineKeyboardMarkup(row_width=1)
    if prefix in ["sel_src", "sel_tgt"]:
        markup.add(InlineKeyboardButton("👤 Select Userbot", callback_data=f"pair_userbots_list|{prefix}"))
    ...
    async for dialog in target_client.iter_dialogs(limit=100):
    ...
    cancel_callback = "dash_main"
    if prefix in ["sel_src", "sel_tgt"]:
        cancel_callback = "pairs_main"
    elif prefix == "kw_grp_add":
        cancel_callback = "kw_mon_groups"
    elif prefix == "pm_tgt":
        cancel_callback = "pm_fwd_main"
    elif prefix == "pm_mon_tgt":
        cancel_callback = "pm_mon_settings"
            ...
            if cmd_start in [".login", "/login"] or m.sender_id in userbot_login_states:
                if cmd_start in [".login", "/login"]:
                    if len(parts) >= 4:
                        ...
                        userbot_login_states[m.sender_id] = {"state": "awaiting_phone", "retries": 0}
                    ...
                    if current_state == "awaiting_phone":
                        ...
                        # Fetch default API ID and Hash (Prioritize current client's API ID and Hash)
                        default_api_id = getattr(client, "api_id", None)
                        default_api_hash = getattr(client, "api_hash", None)
                        ...
                        try:
                            ...
                            retries = state_data.get("retries", 0) + 1
                            state_data["retries"] = retries
                            if retries >= 3:
                                await event.reply(f"❌ **OTP verification failed ({retries}/3 attempts):** {e}\n\nToo many failed attempts. The login process has been cancelled. Please restart the process using `.login`.")
                                userbot_login_states.pop(m.sender_id, None)
                            else:
                                await event.reply(f"❌ **OTP verification failed ({retries}/3 attempts):** {e}\n\nPlease try again by entering your correct OTP code:")
                        return
                        ...
                        try:
                            ...
                            state_data["retries"] = retries
                            if retries >= 3:
                                await event.reply(f"❌ **Password verification failed ({retries}/3 attempts):** {e}\n\nToo many failed attempts. The login process has been cancelled. Please restart the process using `.login`.")
                                userbot_login_states.pop(m.sender_id, None)
                            else:
                                await event.reply(f"❌ **Password verification failed ({retries}/3 attempts):** {e}\n\nPlease try again by entering your correct Cloud Password:")
                        return
...
def cmd_backup(message):
    if message.from_user.id != ADMIN_ID: return
    try:
        status_msg = bot.reply_to(message, "⏳ *Generating database backup...*")
        backup_str = generate_db_backup()
        filename = f"userbot_backup_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.json"
        with open(filename, "w", encoding="utf-8") as f:
            f.write(backup_str)
        ...
        with open(filename, "rb") as f:
            bot.send_document(
                message.chat.id,
                f,
                caption="📦 **Database Backup Export**\n\nTo restore this backup, reply to this JSON file with `/restore`."
            )
        os.remove(filename)
        try: bot.delete_message(message.chat.id, status_msg.message_id)
        except Exception: pass
...
def cmd_restore(message):
    ...
    if not message.reply_to_message or not message.reply_to_message.document:
        bot.reply_to(message, "💡 *Usage:* Reply to a backup `.json` file with `/restore` to import it.")
        return
        
    try:
        doc = message.reply_to_message.document
        if not doc.file_name.endswith('.json'):
            bot.reply_to(message, "❌ Please reply to a valid `.json` backup file.")
            return
            
        status_msg = bot.reply_to(message, "⏳ *Downloading and verifying backup file...*")
        
        file_info = bot.get_file(doc.file_id)
        downloaded_file = bot.download_file(file_info.file_path)
        
        backup_str = downloaded_file.decode('utf-8')
        
        data = json.loads(backup_str)
        if not isinstance(data, dict):
            raise ValueError("Invalid backup format")
            
        bot.edit_message_text("⏳ *Restoring database tables...*", message.chat.id, status_msg.message_id)
        
        restore_db_backup(backup_str)
        
        bot.edit_message_text("✅ **Database restored successfully!**\n\nAll settings, userbots, managers, and target pairs have been imported.\n\n⚠️ **IMPORTANT:** Please restart the bot container to apply the restored configurations.", message.chat.id, status_msg.message_id, parse_mode="Markdown")
    except Exception as e:
        ...
        if parts[2] == "page":
            ...
            async def handle_src():
                try:
                    ...
                    if is_forum:
                        markup = await get_topic_selection_markup(sid, "sel_src_topic", client=client)
                        bot.edit_message_text(f"🧵 *『 {getattr(full_chat, 'title', 'Forum')} 』*\nSelect a source topic:", call.message.chat.id, call.message.message_id, reply_markup=markup, parse_mode="Markdown")
                    else:
                        if uid not in login_data:
                            login_data[uid] = {}
        ...
        if parts[2] == "page":
            ...
            async def handle_tgt():
                try:
                    ...
                    if is_forum:
                        markup = await get_topic_selection_markup(tid, "sel_tgt_topic", client=client)
            ...
            try:
                ...
                if isinstance(entity, (Channel, Chat)):
                    ...
                    if not ref_str.replace("-", "").isdigit():
                        extract_and_save_group_link(entity, ref_str)
            except Exception:
                pass
        return entity
    except Exception as e:
